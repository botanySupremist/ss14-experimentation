...
petting-success-synth-cat = You pet {THE($target)} on {POSS-ADJ($target)} rubbery little head.
...
petting-failure-synth-cat = You reach out to pet {THE($target)}, but {SUBJECT($target)} hisses and archs {POSS-ADJ($target)} back! Smells like cut grass.
